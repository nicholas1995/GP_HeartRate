...
void high_isr(void)
{
    ...
    if(INTCONbits.INT0IF==1)              //RB0 changed states.
    {
        ...
        {
            count_RB++;
            //PORTCbits.RC5= !PORTCbits.RC5; //anytime the input switches from low to high RB7 will switch states
            allow_reading=BLOCK;
        }   
        INTCONbits.INT0IF=0;
    }
...
void main(void)
{
    ...
    while(1)
    {
        ...
        {      
            //PORTCbits.RC6=! PORTCbits.RC6;          //USED TO ENSURE THAT IT IS TESTING EVERY 5 SECONDS 
            bpm=BPM();
            itoa(bpm,bpm_out);
